...
var _ = {
  ...
  any: require('lodash-compat/collection/any')
};
...
Operation.prototype.rfc6570CompliantUrl = function(path) {
  var queryStringVariables = '';
  var noQueryParamsPath = this.stripQueryStringVariables(this.stripAnchors(path));
  var queryParams = _.filter(this.parameters, function (param) {
    return param.in === 'query';
  });
  if (queryParams.length > 0) {
      if (noQueryParamsPath.indexOf('?') >= 0) {
  ...
  for (var i = 0; i < queryParams.length; i++) {
    ...
    queryStringVariables += this.encodeQueryParam(param.name) + paramSuffix;
    queryParamCount++;
  }
  ...
  return noQueryParamsPath + queryStringVariables;
};

Operation.prototype.stripAnchors = function (path) {
...
Operation.prototype.stripQueryStringVariables = function (path) {
  var pattern = /{(\w*)([\?\&]\w+(,\w+)*)}/g;
  return path.replace(pattern, '{$1}').replace('{}', '').replace('}{', '');
};

function maybeTransformValue(paramValue, param) {
  if (Array.isArray(paramValue) && !_.isUndefined(param)) {
    if (param.collectionFormat === 'pipes') {
      return paramValue.join('|');
    } else if (param.collectionFormat === 'tsv') {
      return paramValue.join('\\t');
    } else if (param.collectionFormat === 'csv') {
      return paramValue.join(',');
...
function findParamByName(name, params) {
  return _.find(params, function(p) {
    return p && name === p.name;
  });
}

Operation.prototype.transformArgs = function(args, params) {
  var transformed = {};
  _.each(_.keys(args), function(key) {
    var param = findParamByName(key, params);
    if (param) {
      // if (param.in === 'query') {
        transformed[encodeURIComponent(key)] = maybeTransformValue(args[key], param);
      // } else {
      //   transformed[encodeURIComponent(key)] = "{".concat(param.name).concat("}")
      // }
    }
  });
  return transformed;
...
Operation.prototype.urlify = function (args) {
  var rfc6570 = this.rfc6570CompliantUrl(this.path);
  var template = UriTemplate.parse(rfc6570);
  var url = this.scheme + '://' + this.host;

  if (this.basePath !== '/') {
    url += this.basePath;
  }
  return url + this.unTransformKeys(template.expand(this.transformArgs(args, this.parameters)))
                  .replace(new RegExp('%2C', 'g'), ',')
                  .replace(new RegExp('%7C', 'g'), '|')
                  .replace(new RegExp('%5Ct', 'g'), '%09')
                  .replace(new RegExp('%7B', 'g'), '{')
                  .replace(new RegExp('%7D', 'g'), '}');
};

Operation.prototype.unTransformKeys = function (url) {
  var i;
  var unTransformed = url;
  for (i = 0; i < this.parameters.length; i++) {
    var from = encodeURIComponent(this.parameters[i].name).concat("=");
    var to = this.parameters[i].name.concat("=");
    unTransformed = unTransformed.replace(from, to);
  }
