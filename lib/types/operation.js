...
Operation.prototype.rfc6570CompliantUrl = function(path) {
  ...
  var noQueryParamsPath = this.stripQueryStringVariables(path);
...
function transformArgs(args, params) {
  var transformed = {};
  _.each(_.keys(args), function(key) {
    var param = findParamByName(key, params);
    transformed[key] = maybeTransformValue(args[key], param);
  });
  return transformed;
}
...
Operation.prototype.urlify = function (args) {
  ...
  return url + template.expand(transformArgs(args, this.parameters))
...
Operation.prototype.asCurl = function (args1, args2) {
  ...
  return 'curl ' + (results.join(' ')) + ' \'' + this.urlify(args1) + '\'';
};
