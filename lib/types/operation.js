...
Operation.prototype.withTemplatizedQueryString = function(path) {
  var querystring = "";
  ...
  for (var i = 0; i < queryParams.length; i++) {
    ...
    if (param.in === 'query') {
      ...
      querystring += this.encodeQueryParam(param.name) + paramSuffix;
      queryParamCount++;
    }
  }
  ...
  return this.normalizedRfc6570Url(path) + querystring;
};

Operation.prototype.normalizedRfc6570Url = function (path) {
  var pattern = /{(\w*)([\?\&]\w+(,\w+)+)}/g;
  return path.replace(pattern, '{$1}').replace('{}', '');
};

Operation.prototype.urlify = function (args) {
  var pathWithTemplatedQuery = this.withTemplatizedQueryString.call(this, this.path);
  var template = UriTemplate.parse(pathWithTemplatedQuery);
